...
object DetectionManager extends Database with AmazonS3 {
  ...
  def getCodeFilePairs (matchIndex: String, resultFolderName: String): List[CodeFilePair] = {
    ...
    val studentAMatchFile = s"${currentDetection.get.destinationPath}/$resultFolderName/match" + matchIndex + "-0.html"
    val studentBMatchFile = s"${currentDetection.get.destinationPath}/$resultFolderName/match" + matchIndex + "-1.html"
    val comparisonTable = s"${currentDetection.get.destinationPath}/$resultFolderName/match" + matchIndex + "-top.html"
